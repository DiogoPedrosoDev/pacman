...
#include "Codigo/MovimentacaoAleatoria.h"
#include "Codigo/MovimentacaoInteligente.h"
...
int main(int argc, char** argv) {
    ...
    ALLEGRO_BITMAP* bmpinimigo = al_load_bitmap("Imagens/InimigosSprite.png");
    ...
    // Criação do bitmap redimensionado Jogador
    int novaLargura = 150;
    int novaAltura = 120;
    ...
    // Criação do bitmap redimensionado Inimigos
    int novaLarguraInimigos = 120;
    int novaAlturaInimigos = 30;
    ALLEGRO_BITMAP* imagemRedimensionadaInimigos = al_create_bitmap(novaLargura, novaAltura);
    if (!imagemRedimensionadaInimigos) {
        fprintf(stderr, "Falha ao criar o bitmap redimensionado.\n");
        al_destroy_bitmap(bmpinimigo);
        return -1;
    }
    ...
    //Redimencionamento do Bitmap dos Inimigos

    // Configuração do bitmap redimensionado como o alvo de desenho
    al_set_target_bitmap(imagemRedimensionadaInimigos);

    // Desenho da imagem original no bitmap redimensionado
    al_draw_scaled_bitmap(bmpinimigo, 0, 0, al_get_bitmap_width(bmpinimigo), al_get_bitmap_height(bmpinimigo),
        0, 0, novaLarguraInimigos, novaAlturaInimigos, 0);

    // Restauração do alvo de desenho
    al_set_target_backbuffer(display);

    MovimentacaoAleatoria fantasmas2(420, 690, imagemRedimensionadaInimigos), fantasmas3(480, 690, imagemRedimensionadaInimigos), fantasmas4(510, 690, imagemRedimensionadaInimigos);
    MovimentacaoInteligente fantasmas1(390, 690, imagemRedimensionadaInimigos);

    Inimigos* VetInimigos[4];

    VetInimigos[0] = &fantasmas1;
    ...
    while (running) {
        ...
        for (int i = 0; i < 4; i++)
        {
            VetInimigos[i]->CalcularPosicao();
            VetInimigos[i]->SorteioDirecao(mapa, Jogador.getPacmanX(), Jogador.getPacmanY());

            VetInimigos[i]->ComandosInimigos(mapa);
            VetInimigos[i]->MovimentacaoInimigos(mapa, false);
        }
        ...
        if (re_desenha && al_is_event_queue_empty(event_queue)) { 
           ...
           //Desenha os inimigos
            ...
            for (int i = 0; i < 4; i++)
            {
                VetInimigos[i]->DesenhaInimigos(0, i);
            }
        ...
        for (int i = 0; i < 4; i++)
        {
            if (Jogador.getPacmanX() == VetInimigos[i]->GetInimigosX() && Jogador.getPacmanY() == VetInimigos[i]->GetInimigosY())
            {
                running = false;
            }
        }

        if (Jogador.getPlacar() == 79)
        {
            running = false;
        }
    }
